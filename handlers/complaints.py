...
def register(bot):
    ...
    def view_complaints(call):
        ...
        for comp in complaints:
            ...
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("✉️ الرد", callback_data=f"reply_comp:{comp['_id']}"))
            markup.add(InlineKeyboardButton("✅ تم الحل", callback_data=f"resolve_comp:{comp['_id']}"))
    ...
    def ask_reply(call):
        ...
        bot.send_message(call.message.chat.id, "✏️ اكتب الرد الآن:")
        bot.register_next_step_handler(call.message, lambda msg: send_reply(msg, comp_id))

    def send_reply(msg, comp_id):
